...
class RoomHistory(models.Model):
	...
	# 	return SingleRoomEntry.objects.filter(room = room, has_left = False, join_date__gt = time).user_set.all()

	# @staticmethod
	# def getVistedRooms(user,time = "1970-01-01T00:00+00:00" ):
	# 	return  SingleRoomEntry.objects.filter(user = user, join_date__gt = time).room_set.all()
